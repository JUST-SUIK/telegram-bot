...
用户："什么是 Python" -> {"action": "ask", "params": "什么是 Python", "response": "好的，正在向 AI 提问..."}
用户："你好" -> {"action": "chat", "params": "", "response": "你好！有什么我可以帮助你的吗？"}
用户："今天天气怎么样" -> {"action": "chat", "params": "", "response": "我无法查询天气，但你可以告诉我你需要什么帮助？"}

重要规则：
1. 只返回 JSON，不要有其他内容
2. 如果用户询问电脑状态、运行情况、正在做什么等，必须执行 "status" 或 "list" 操作
3. 只有真正的闲聊才使用 "chat" 操作
4. 如果用户的问题需要调用 AI，使用 "ask" 操作
5. response 字段要简洁友好
"""
...
async def handle_natural_language(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # For chat action, response was already sent above